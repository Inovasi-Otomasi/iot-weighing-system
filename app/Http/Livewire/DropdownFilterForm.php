...
class DropdownFilterForm extends Component
{
    ...
    {
        $data = [
            'sku_list' => DB::table('historical_log')->select('sku_name')->distinct()->where('sku_name', 'like', '%' . $this->sku_search . '%')->get(),
            'line_list' => DB::table('historical_log')->select('line_name')->distinct()->where('line_name', 'like', '%' . $this->line_search . '%')->get(),
            'machine_list' => DB::table('historical_log')->select('machine_name')->distinct()->where('machine_name', 'like', '%' . $this->machine_search . '%')->get(),
            'shift_list' => DB::table('historical_log')->select('shift_name')->distinct()->where('shift_name', 'like', '%' . $this->shift_search . '%')->get(),
            'group_list' => DB::table('historical_log')->select('shift_group')->distinct()->where('shift_group', 'like', '%' . $this->group_search . '%')->get(),
            'user_list' => DB::table('historical_log')->select('user')->distinct()->where('user', 'like', '%' . $this->user_search . '%')->get(),
